...
class ObjectDetectionMetric():
    ...
    def _get_IOUs(bboxes1_xyxy, bboxes2_xyxy):
        ...
        split = len(bboxes1_xyxy)
        bboxes = np.concatenate((bboxes1_xyxy, bboxes2_xyxy), axis=0)
        bboxes1 = bboxes[:split].reshape((-1, 1, 4))
        bboxes2 = bboxes[split:].reshape((1, -1, 4))
        ...
        intersection = np.maximum(x_overlap, 0) * np.maximum(y_overlap, 0)
        area1 = (bboxes1[:, :, 2] - bboxes1[:, :, 0]) * (bboxes1[:, :, 3] - bboxes1[:, :, 1])
        area2 = (bboxes2[:, :, 2] - bboxes2[:, :, 0]) * (bboxes2[:, :, 3] - bboxes2[:, :, 1])
...
if __name__ == "__main__":
    ...
    bboxes_prediction  = np.array([[000,100,100,200],
                          [000,100,100,201]])
    labels_prediction  = np.array([0,0])
    scores_prediction  = np.array([20,30])
    
    
    bboxes_groundtruth = np.array([[10,110,110,210],
                          [000,100,100,201]])
    labels_groundtruth = np.array([0,1])

    metric = ObjectDetectionMetric([0,1,23])
